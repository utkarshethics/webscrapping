...
class ImmutableDict(util.Mapping):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            is_dict and not all([isinstance(v, util.Hashable) for v in list(arg.values())]) or
...
class Namespaces(ImmutableDict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        arg = args[0] if args else kwargs
        is_dict = isinstance(arg, dict)
        if is_dict and not all([isinstance(k, util.string) and isinstance(v, util.string) for k, v in list(arg.items())]):
